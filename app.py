...
def get_outfit_suggestion_fallback(color_name):    # this is the old function, rn using AI but keeping this as a backup
    suggestions = {
...
def pil_image_to_part(image: Image.Image) -> types.Part:
    """Converts a PIL image into a Part object that can be sent to Gemini."""
    buffer = io.BytesIO()
...
def get_ai_outfit_suggestion(
    ...
    predictions_text = ", ".join(
        f"{label} ({confidence * 100:.1f}%)" for _, label, confidence in top_predictions
    )

    prompt = f"""
You are a professional personal stylist. Below is a clothing/accessory image
along with some automatically detected information about it:

- Detected object type: {detected_name or "unknown"}
- Dominant color: {color_name} (hex: {hex_color})
- Image classification predictions: {predictions_text}

Carefully examine the image as well (consider details such as fabric, pattern,
style, cut, and seasonal suitability) and, combining this with the information
above, write a short, friendly, and practical outfit suggestion for the user.
Your response should:
- Be in English
- Be 3-5 sentences long
- Mention which colors/pieces it can be paired with, and which setting/season
  it suits
- Avoid lists or markdown headers, just write a flowing paragraph
"""
    ...
    try:
        response = gemini_client.models.generate_content(
            ...
            contents=[
                prompt,
                pil_image_to_part(clothing_image),
            ],
        )
        text = (response.text or "").strip()
        return text if text else None
    except Exception as e:
        st.caption(f"⚠️ Couldn't get a Gemini suggestion, showing fallback instead. ({e})")
        return None
...
if uploaded_file is not None:
    ...
    image_for_ai = clothing_area if clothing_area is not None else image

    with st.spinner("AI stylist is thinking..."):
        ai_suggestion = get_ai_outfit_suggestion(
            clothing_image=image_for_ai,
            detected_name=detected_name,
            color_name=color_name,
            hex_color=hex_color,
            top_predictions=decoded_predictions,
        )

    if ai_suggestion:
        st.write(ai_suggestion)
        st.caption("✨ This suggestion was generated by Gemini AI.")
    else:
        st.write(get_outfit_suggestion_fallback(color_name))
        if gemini_client is None:
            st.caption(
                "ℹ️ GEMINI_API_KEY is not set, so a fallback (rule-based) "
                "suggestion is shown instead."
            )
